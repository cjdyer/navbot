...
public:
    AsyncHandler()
    {
        ...
        Log::log_info("AsyncHandler::AsyncHandler - Parent constructed");
    }

    ~AsyncHandler()
    {
        stop_handle_thread();
        Log::log_info("AsyncHandler::~AsyncHandler - AsyncHandler terminated gracefully");
    }
    
    virtual void handle_trigger_action(T& t) = 0;

protected:
    void trigger_handler_thread(const T &trigger_action_struct)
    {
        { // Scope for mutex
    ...
    void stop_handler()
    {
        Log::log_info("AsyncHandler::Stop - stop_handle_thread");
        stop_handle_thread();
    }
...
private: // functions
    ...
    void run_handle_thread()
    {
        Log::log_info("AsyncHandler::run_handle_thread - handle thread started");
        ...
        while (m_handle_thread_running)
        {
            { // Scope for mutex
                ...
                m_handle_thread_cond_var.wait(lk,
                                              [this]
                                              { return ((!m_trigger_action_queue.empty()) ||
                                                        (!m_handle_thread_running)); });
            }

            if (!m_handle_thread_running)
            {
                Log::log_info("AsyncHandler::run_handle_thread - handle thread terminated gracefully");
                return;
            }
            ...
            while (!m_trigger_action_queue.empty())
            {
                ...
                {
                    Log::log_info("handle - ERROR! Failed to pop an action request from the queue");
                }
            }
        }
